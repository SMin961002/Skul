...
void ProjectileHeadSkull::Init()
{
	...
	m_speed = 10;
	m_rot = 0;
	SetObject(OBJECTMANAGER->AddObject("BasicSkul_HeadSkullProjectile", 0, 0, ePlayer));
	m_obj->AddComponent<PixelCollisionComponent>()->setting(SCENEMANAGER->m_tiles, &m_obj->x, &m_obj->y);
	m_obj->AddComponent<RigidBodyComponent>();
	m_obj->GetComponent<RigidBodyComponent>()->SetGravityOnOff(false);
	m_colHeadSkull = m_obj->AddComponent<CollisionComponent>();
	m_colHeadSkull->Setting(14, m_obj->x, m_obj->y, "ThrowHeadSkull");
...
{
	IMAGEMANAGER->CenterRender(m_img, m_obj->x, m_obj->y, 2, 2,0, m_isReverse);
}
