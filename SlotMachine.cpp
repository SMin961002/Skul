...
void SlotMachine::Init()
{
	...
	m_printResult = false;
	m_actionThunder = false;
	m_actionExplosion = false;

	m_delay = 0;
	m_alpha = 1.5;

	m_thunder = nullptr;
	//int tmp = MY_UTILITY::getInt(3);	//현재는 폭발 2칸, 3칸짜리 결과만 사용하므로 2로 제한함
	int tmp = 2;
	switch (tmp)
	{
	case 0:	//빨강2, 노랑1 - 폭발 + 썬더
	...
	case 1:	//빨강3 - 빨강 대성공
		...
		m_resultExplosion = 3;
		break;
	case 2:	//노랑2, 파랑1 - 썬더2, 레이저(레이저 미구현)
		SOUNDMANAGER->FindSound("SlotMachineRunning")->Play(false);
...
void SlotMachine::Update()
{
	...
	else if (m_printResult)
	{
		...
		else if (m_delay < 1.0)	//폭발효과 발동
		{
			if (m_resultExplosion > 0)
				if (m_actionExplosion == false)
				{
					m_actionExplosion = true;
		...
		else if (m_delay < 2)	//번개효과 발동
		{
			if (m_actionThunder == false)
			{
				m_actionThunder = true;
				//Thunder
				if (m_resultThunder > 0 && m_thunder != nullptr)
					m_thunder->ShotStart();
			}
		}
	}
...
void SlotMachine::Render()
{
	...
	if (m_actionExplosion && m_resultExplosion > 0)
	{
